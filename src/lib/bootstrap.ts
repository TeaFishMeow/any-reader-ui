import type { AppConfig, CanvasState, QARecord, WorkspaceSnapshot } from '../types/domain'
...
interface BootstrapModule {
  ...
  purgeQaRecord(recordId: string): Promise<void>
}
...
export async function saveConfig(config: AppConfig) {
  await (await loadBootstrapModule()).saveConfig(config)
}

export async function saveCanvas(canvas: CanvasState) {
  await (await loadBootstrapModule()).saveCanvas(canvas)
}
...
export async function purgeQaRecord(recordId: string) {
  await (await loadBootstrapModule()).purgeQaRecord(recordId)
}
